...
const ToggleSidebar = () =>
  // { isExpanded, setIsExpanded }: SidebarProps
  {
    ...
    return (
      <div
        className={`h-screen overflow-hidden relative flex flex-col bg-gray-50 ease-in-out transition-all duration-300 ${
          ...
          {sideTabs.map((item) => {
            ...
            return (
              <Link href={item.slug} key={item.title}>
                <button
                  className={`flex items-center w-full gap-3 p-3 rounded-lg hover:bg-(--surface-muted) text-surface-foreground cursor-pointer ${pathname === item.slug && "bg-(--surface-muted)"}`}
                >
                  <Icon />
                  {isExpanded && (
